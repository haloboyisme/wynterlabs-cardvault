  ...
  return <details className="presentation-panel" open={account || undefined}>
    ...
    {!state ? <p role="status">{status||"Loading presentation settings…"}</p> : <>
      ...
      {!account && <section className="studio-preview-card" aria-label="Live card presentation">
        ...
        <Stage key={revealReplay} state={{...state,settings:draft,preview:!replay?preview:undefined,cards:lastReplay??((!replay&&preview)?[preview]:state.cards)}} replay={replay}/>
      ...
      <section className="studio-sound-card" aria-label="Sound settings">
        ...
        <div className="presentation-controls">
          ...
          {select("audio","Play sound in",["scanner","overlay"])}{select("found","Card found sound",SOUND_OPTIONS)}{select("confirm","Card accepted sound",SOUND_OPTIONS)}{select("reject","Rejection sound",SOUND_OPTIONS)}{select("complete","Pack-complete sound",SOUND_OPTIONS)}
        </div><p className="studio-helper">15 built-in sounds and alerts, plus Off. Choose one sound destination to avoid hearing it twice. Master mute is saved to your account; preview mute only affects this tab.</p>
      ...
      {!account && <>
        ...
        <p>{state.cards.length} saved pulls · No fixed scan limit · {state.finished ? "Pack finished. Start a new pack to capture more." : "Pack in progress"}</p>
        <div className="presentation-actions">
          <button disabled={busy||pending>0||!state.cards.length} onClick={()=>void run(()=>event("finish"))}>Finish Pack & replay</button>
        ...
        <p>Finish Pack automatically replays saved cards. The next accepted card starts a fresh pack. Replay last session restores the previous recap after a refresh or restart without changing your collection. A new pack revokes its old OBS link; create a new link to share the next session.</p>
