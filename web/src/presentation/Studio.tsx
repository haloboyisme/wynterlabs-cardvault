...
export function Studio({ account = false }: { account?: boolean }) {
  ...
  const [section, setSection] = useState<"look" | "sounds" | "replay">("look");
  ...
  const [lastReplay, setLastReplay] = useState<Pull[] | null>(null);
  ...
  async function drain() {
    ...
    try {while(queue.current.length && alive.current){
      ...
      queue.current.shift();setPending(queue.current.length);setState(s);setLastReplay(null);setReplay(false);
  ...
  useEffect(() => {if(account)return;const listener=(e:Event)=>{const card=(e as CustomEvent).detail as Pull|null;setPreview(card);if(card){setLastReplay(null);setReplay(false);}const key=card?`${card.printing_id}|${card.preview_key??""}`:"";if(key===foundKey.current)return;foundKey.current=key;const current=stateRef.current;if(!card||!current)return;
  ...
  async function event(kind:string,extra:object={}) {const next=await requestEvent(kind,extra);setState(next);setLastReplay(null);setRevealReplay(v=>v+1);setReplay(kind==="finish"||kind==="replay");play(next);if(kind==="new")setUrl("");}
  ...
  return <details className="presentation-panel studio-redesign" open={account || undefined}>
    <summary><span>{account ? "Streamer settings and sound" : "Streamer preview"}</span><small>Your cards, sounds & replay</small></summary>
    <p className="studio-intro">Pick a look → choose your sounds → save. Then scan! Cards join your replay after you confirm and add them to your collection.</p>
    {!state ? <p role="status">{status||"Loading presentation settings…"}</p> : <>
      ...
      <header className="studio-brand-header"><div><small>WYNTERLABS · CARDVAULT</small><h2>Your pulls. Your show.</h2><p>Make a great reveal in just a few clicks.</p></div><span className="studio-brand-mark" aria-hidden="true">W</span></header>
      ...
      {!account && <section className="studio-preview-card" aria-label="Live card presentation">
        ...
        <Stage key={revealReplay} state={{...state,settings:draft,preview:!replay?(preview??(!state.cards.length?{id:"sample",printing_id:"sample",name:"Your next great pull",image:"",set:"Sample set",number:"001",language:"en",finish:"nonfoil",price:"$—",rarity:"Demo",quantity:1,highlight:false}:undefined)):undefined,cards:lastReplay??((!replay&&preview)?[preview]:state.cards)}} replay={replay}/>
        <div className="studio-preview-caption"><span>{!replay&&preview?"Selected printing · confirm it before saving":(!state.cards.length?"Sample preview · not added to your pack":"Saved pulls · ready for your audience")}</span><button type="button" onClick={()=>setRevealReplay(v=>v+1)}>Replay animation</button></div>
        <p className="studio-helper">This is a preview. Animations never confirm cards or move the feeder.</p>
      </section>}
      <nav className="studio-navigation" aria-label="Streamer controls">
        {(["look","sounds",...(!account?["replay"]:[])] as const).map((value,index)=><button type="button" key={value} aria-pressed={section===value} onClick={()=>setSection(value as typeof section)}><span>{index+1}</span><strong>{value==="look"?"Look":value==="sounds"?"Sounds":"Replay & OBS"}</strong><small>{value==="look"?"Style your reveal":value==="sounds"?"Make some noise":"Keep the moment"}</small></button>)}
      </nav>
      <section hidden={section!=="look"} className="studio-settings-card" aria-label="Preview settings">
        <div className="studio-section-title"><div><small>STEP 1</small><h3>Choose a look</h3></div></div>
        <div className="presentation-actions studio-presets"><button type="button" onClick={()=>setDraft({...defaults,enabled:draft.enabled})}><span className="studio-preset-art preset-plain" aria-hidden="true">A</span>Plain preset<small>No animation</small></button><button type="button" onClick={()=>setDraft({...draft,reveal:"fade",glow:false,pack:false})}><span className="studio-preset-art preset-fade" aria-hidden="true">A</span>Subtle preset<small>A gentle fade</small></button><button type="button" onClick={()=>setDraft({...draft,reveal:"flip",glow:true,pack:true})}><span className="studio-preset-art preset-flip" aria-hidden="true">W</span>Streamer preset<small>Flip, glow & fun</small></button></div>
        <div className="presentation-controls">
          {check("enabled","Add saved cards to my replay")}{select("reveal","Card animation",["instant","fade","flip","slide","zoom","pop","tilt"])}
          {select("background","Background",["transparent","green","blue","solid","gradient","spotlight","grid","stars"])}{select("layout","Screen shape",["landscape","portrait","square","reverse"])}
          {check("details","Show set and printing details")}{check("prices","Show price")}
        </div>
        <details className="studio-more"><summary>More look options <small>Card back, colors, speed & effects</small></summary><div className="presentation-controls">
          ...
          <label>Animation length (seconds)<input type="number" min="0.1" max="3" step="0.1" value={draft.duration} onChange={e=>update("duration",Number(e.target.value))}/></label><label>Time on screen (seconds)<input type="number" min="1" max="10" value={draft.hold} onChange={e=>update("hold",Number(e.target.value))}/></label><label>Card size<input type="range" min="0.5" max="1.5" step="0.05" value={draft.scale} onChange={e=>update("scale",Number(e.target.value))}/></label>
          {check("glow","Card glow")}{check("pack","Pack-opening intro")}
        </div></details>
      </section></div>
      <details open hidden={section!=="sounds"} className="studio-sound-card studio-step"><summary><span>2 · Sounds</span><small>Volume, alerts & celebrations</small></summary><section aria-label="Sound settings">
        <div className="studio-section-title"><div><small>STEP 2</small><h3>Choose your sounds</h3></div><span className="studio-tag">{draft.muted ? "Muted" : "On"}</span></div>
        <div className="presentation-controls">
          {check("muted","Mute all account sounds")}<label>Volume<input type="range" min="0" max="1" step="0.05" value={draft.volume} onChange={e=>update("volume",Number(e.target.value))}/><small>{Math.round(draft.volume*100)}%</small></label>
          {select("audio","Play sound in",["scanner","overlay"])}</div><details className="studio-more"><summary>Choose each alert sound <small>Found, saved, rejected & finished</small></summary><div className="presentation-controls">{select("found","Card found sound",SOUND_OPTIONS)}{select("confirm","Card accepted sound",SOUND_OPTIONS)}{select("reject","Rejection sound",SOUND_OPTIONS)}{select("complete","Pack-complete sound",SOUND_OPTIONS)}
        </div><p className="studio-helper">Pick from 15 sounds, or choose Off. “Found” means a card was recognized. “Accepted” means you saved it. Sounds play in one place so you do not hear them twice.</p>
        <div className="presentation-actions">{(["found","confirm","reject","complete"] as const).map(kind=><button key={kind} type="button" onClick={()=>void enableAudio().then(()=>{if(!temporaryMute&&audio.current)tone(audio.current,draft,kind);})}>Try {kind === "confirm" ? "accepted" : kind === "reject" ? "rejection" : kind === "complete" ? "pack-complete" : "found"} sound</button>)}</div>
      </details></section>
      <details className="studio-prize-card"><summary>Celebrate valuable cards <small>$5, $10, $20, $50 & $100</small></summary><h3>Prize pulls</h3><p className="studio-helper">A saved card gets the highest celebration it qualifies for. For example, a $25 card gets the $20 celebration. Prices are per card in USD. Muting your account also mutes these sounds.</p><div className="presentation-controls">{check("rewards","Prize celebrations")}{check("particles","Confetti & sparks")}</div><div className="prize-tier-grid">{(draft.reward_tiers??rewardDefaults).map((tier,index)=><details key={tier.threshold} className="studio-tier"><summary>${tier.threshold}+ card <small>{tier.enabled ? "On" : "Off"}</small></summary><fieldset><legend>${tier.threshold}+ celebration</legend><label><input type="checkbox" checked={tier.enabled} onChange={e=>update("reward_tiers",(draft.reward_tiers??rewardDefaults).map((t,i)=>i===index?{...t,enabled:e.target.checked}:t))}/>Enable this tier</label><label>Prize message<input maxLength={40} value={tier.label} onChange={e=>update("reward_tiers",(draft.reward_tiers??rewardDefaults).map((t,i)=>i===index?{...t,label:e.target.value}:t))}/></label>{(["sound","effect"]as const).map(key=><label key={key}>{key==="sound"?"Prize sound":"Prize effect"}<select value={tier[key]} onChange={e=>update("reward_tiers",(draft.reward_tiers??rewardDefaults).map((t,i)=>i===index?{...t,[key]:e.target.value}:t))}>{(key==="sound"?SOUND_OPTIONS:["none","confetti","sparks","burst"]).map(value=><option key={value} value={value}>{value[0].toUpperCase()+value.slice(1)}</option>)}</select></label>)}<button type="button" disabled={busy||!tier.enabled||tier.sound==="off"} onClick={()=>void enableAudio().then(()=>{if(!temporaryMute&&audio.current)tone(audio.current,draft,"confirm",undefined,{price:`$${tier.threshold}`} as Pull);})}>Test ${tier.threshold} jingle</button></fieldset></details>)}</div></details></details>
      <div className="presentation-actions studio-save-bar"><button className="button primary" type="button" disabled={busy} onClick={()=>void run(async()=>{const next=await apiRequest<Presentation>(endpoint+"/settings",{method:"PUT",body:JSON.stringify(draft)});setState(next);setDraft(next.settings);setStatus("Account settings saved.");})}>Save presentation settings</button><details className="studio-more"><summary>Preview audio tools</summary><div className="presentation-actions"><button type="button" onClick={()=>void enableAudio()}>Enable browser audio</button><button type="button" onClick={()=>setTemporaryMute(v=>!v)}>{temporaryMute?"Unmute this preview":"Mute this preview"}</button><button type="button" onClick={()=>void enableAudio().then(()=>{if(!temporaryMute&&audio.current)tone(audio.current,draft,"confirm");})}>Test accepted sound</button><button type="button" onClick={()=>void enableAudio().then(()=>{if(!temporaryMute&&audio.current)tone(audio.current,draft,"found");})}>Test found sound</button></div></details></div>
      {!account && <>
        <section hidden={section!=="replay"} className="studio-pack-card" aria-label="Pack recap controls"><div className="studio-section-title"><div><small>STEP 3</small><h3>Replay & save</h3></div><span className="studio-tag">{state.cards.length} pulls</span></div>
        <p>{state.cards.length} saved pulls · No fixed scan limit · {state.finished ? "Finished. Your next saved card starts a new pack." : "Ready for more cards"}</p>
        <p className="studio-helper">A pack is the group of cards you saved during this scan. Finish it to watch them again, or replay your last session if you refreshed by accident.</p><div className="presentation-actions">
          <button className="button primary" disabled={busy||pending>0||!state.cards.length} onClick={()=>void run(()=>event("finish"))}>Finish Pack & replay</button>
          <button disabled={busy||!state.cards.length} onClick={()=>void run(()=>event("replay"))}>Replay recap</button>
          <button disabled={busy||!state.last_session?.length} onClick={()=>{setLastReplay(state.last_session??null);setPreview(null);setReplay(true);setRevealReplay(v=>v+1);setStatus("Replaying your last session. Your current cards are unchanged.");}}>Replay last session</button>
          <button disabled={busy||pending>0||(!state.finished&&state.cards.length>0)} onClick={()=>void run(()=>event("new"))}>Start new pack</button>
        </div><details className="studio-obs-block studio-more"><summary>Show my cards in OBS <small>Optional · for recording or streaming</small></summary><p className="studio-helper">Create a private viewing link, then add it as an OBS Browser Source.</p><div className="presentation-actions">
          <button disabled={busy} onClick={()=>void run(async()=>{const next=await apiRequest<Presentation>(endpoint+"/settings",{method:"PUT",body:JSON.stringify({...state.settings,audio:"overlay"})});setState(next);setDraft(current=>({...current,audio:"overlay"}));const {token}=await apiRequest<{token:string}>(endpoint+"/link",{method:"POST"});setUrl(`${location.origin}/overlay#${token}`);setStatus("Private view-only link created. OBS audio starts automatically and respects Account master mute. Expires in 7 days; previous links are disabled.");})}>Create / rotate OBS link</button>
        ...
        <p>Your OBS link belongs to this pack. A new pack turns off the old link, so create a new one when you start again.</p>
