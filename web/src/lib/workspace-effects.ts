...
export const effectsDefaults={motion:true,lowPower:true,clicks:false,hover:false,volume:.2,speed:1,cue:"soft"};
export type WorkspaceEffects=typeof effectsDefaults;
export function readEffects():WorkspaceEffects{try{const s=JSON.parse(localStorage.getItem(EFFECTS_KEY)||"{}");return {speed:[.5,1,1.5].includes(s.speed)?s.speed:1,cue:["soft","glass","retro"].includes(s.cue)?s.cue:"soft",motion:typeof s.motion==="boolean"?s.motion:true,lowPower:typeof s.lowPower==="boolean"?s.lowPower:true,clicks:s.clicks===true,hover:s.hover===true,volume:typeof s.volume==="number"&&Number.isFinite(s.volume)?Math.max(0,Math.min(1,s.volume)):.2};}catch{return {...effectsDefaults};}}
