...
beforeEach(() => {
  ...
  vi.stubGlobal("fetch", vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (path === "/api/v1/admin/invitations" && init?.method === "POST") {
      ...
      return new Response(JSON.stringify({ ...invitation, id: `created-invite-${createdInvitationCount}`, raw_token: "one-time-private-token" }), { status: 201, headers: { "content-type": "application/json" } })
    }
...
it("creates member and administrator one-time links without exposing a super administrator option", async () => {
  ...
  const role = screen.getByLabelText(/invitation account type/i)
  expect(within(role).getByRole("option", { name: /member/i })).toBeVisible()
  expect(within(role).getByRole("option", { name: /administrator/i })).toBeVisible()
  expect(within(role).queryByRole("option", { name: /super/i })).not.toBeInTheDocument()
  ...
  const memberCreate = vi.mocked(fetch).mock.calls.find(([path, init]) =>
    String(path) === "/api/v1/admin/invitations" && init?.method === "POST",
  )
  expect(JSON.parse(String(memberCreate?.[1]?.body))).toEqual({ target_role: "member" })

  await user.selectOptions(role, "admin")
  await user.click(screen.getByRole("button", { name: /create invitation link/i }))
  const createRequests = vi.mocked(fetch).mock.calls.filter(([path, init]) =>
    String(path) === "/api/v1/admin/invitations" && init?.method === "POST",
  )
  expect(JSON.parse(String(createRequests[1]?.[1]?.body))).toEqual({ target_role: "admin" })
