...
test "$(query "SELECT version_num FROM alembic_version")" = "0016_trusted_mfa_browser"
test "$(query "SELECT count(*) FROM information_schema.tables WHERE table_name = 'mfa_trusted_browsers'")" = "1"
test "$(query "SELECT must_setup_mfa::int FROM users WHERE owner_slot = 1")" = "1"
