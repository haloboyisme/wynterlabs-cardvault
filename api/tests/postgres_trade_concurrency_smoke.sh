...
suffix="${$}-${RANDOM}"
network_name="wynterlabs-trade-concurrency-${suffix}"
database_container="wynterlabs-trade-concurrency-pg-${suffix}"
database_user="trade_concurrency"
database_name="trade_concurrency"
database_password="test-only-credential-a1b5b315b73f"
...
docker run --rm --network "${network_name}" --volume "${api_root}:/app" --workdir /app \
  ...
  "dd if=/dev/zero of=/tmp/mfa-test-key bs=32 count=1 status=none && pip install -q -e '.[dev]' && alembic upgrade head && python tests/postgres_trade_concurrency_smoke.py"

printf '%s\n' 'postgres-trade-concurrency-smoke-ok'
