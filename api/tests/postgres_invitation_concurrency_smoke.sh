...
suffix="${$}-${RANDOM}"
network_name="wynterlabs-invitation-concurrency-${suffix}"
database_container="wynterlabs-invitation-concurrency-pg-${suffix}"
database_user="invitation_concurrency"
database_name="invitation_concurrency"
database_password="test-only-credential-0be3cea34bb7"
...
docker run --rm --network "${network_name}" --volume "${api_root}:/app" --workdir /app \
  ...
  "dd if=/dev/zero of=/tmp/mfa-test-key bs=32 count=1 status=none && pip install -q -e '.[dev]' && alembic upgrade head && python tests/postgres_invitation_concurrency_smoke.py"
