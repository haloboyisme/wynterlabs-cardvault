...
def test_provider_clients_decode_compressed_multichunk_json(tmp_path, client_type):
    ...
    card = {"id": "compressed-card", "padding": "x" * 2048, "card_sets": [{"set_code": "TST-001"}]}
...
def test_provider_clients_abort_streaming_responses_that_exceed_the_byte_limit(
    ...
    async def exercise():
        ...
        async with httpx.AsyncClient(transport=transport) as http:
            with pytest.raises((RuntimeError, ValueError), match="bounded retries|not a ZIP"):
...
def test_distinct_sets_with_reused_codes_survive_repeat_imports(tmp_path):
    ...
    async def exercise():
        ...
        second["set_id"] = str(uuid.uuid4())
        second["set_name"] = "Another set using the same abbreviation"
        fixture = tmp_path / "duplicate-codes.jsonl"
        fixture.write_text(json.dumps(record) + "\n" + json.dumps(second) + "\n")
        importer = CatalogImporter(settings, factory, source=Source(fixture))
        assert (await importer.refresh()).status == "complete"
        async with factory() as session:
            before = {row.scryfall_id: row.code for row in (await session.scalars(select(CardSet))).all()}
        assert len(set(before.values())) == 2
        assert (await importer.refresh()).status == "complete"
        async with factory() as session:
            after = {row.scryfall_id: row.code for row in (await session.scalars(select(CardSet))).all()}
        assert after == before
        await engine.dispose()
...
def test_all_games_continue_after_one_provider_fails(tmp_path):
    from app.catalog.games import SUPPORTED_GAME_KEYS

    async def exercise():
        settings, engine, factory = await database(tmp_path)
        importer = CatalogImporter(settings, factory)
        called = []
        async def refresh_game(game):
            called.append(game)
            if game == "digimon":
                raise RuntimeError("provider failed")
            return ImportOutcome("complete", None, 1, 0, False)
        importer._refresh_locked = refresh_game
        outcome = await importer.refresh("all")
        assert outcome.status == "partial"
        assert outcome.failed_games == ("digimon",)
        assert called == list(SUPPORTED_GAME_KEYS)
        assert outcome.imported_records == len(SUPPORTED_GAME_KEYS) - 1
        await engine.dispose()
    asyncio.run(exercise())
