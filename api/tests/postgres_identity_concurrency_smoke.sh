...
suffix="${$}-${RANDOM}"
network_name="wynterlabs-identity-race-${suffix}"
database_container="wynterlabs-identity-race-pg-${suffix}"
database_user="identity_race"
database_name="identity_race"
database_password="test-only-credential-f3f89c420ab2"
...
docker run --rm \
  ...
  --env "CARDS_TEST_DATABASE_URL=${database_url}" \
  ...
  "dd if=/dev/zero of=/tmp/mfa-test-key bs=32 count=1 status=none && pip install -q -e .[dev] && alembic upgrade head && pytest -q tests/postgres_identity_concurrency_smoke.py"

echo "ephemeral-postgres-identity-concurrency-smoke-ok"
