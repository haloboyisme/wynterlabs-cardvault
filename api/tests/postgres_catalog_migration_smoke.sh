...
expect "t" "SELECT version_num = '0017_catalog_refresh_schedule' FROM alembic_version;"
expect "1" "SELECT count(*) FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'catalog_refresh_schedules';"
...
run_alembic downgrade 0014_more_tcgjson_games
expect "0014_more_tcgjson_games" "SELECT version_num FROM alembic_version;"
query "INSERT INTO account_invitations (id, token_hash, created_by_user_id, expires_at, revoked_at, used_at, used_by_user_id, revision, created_at, updated_at) VALUES ('88888888-8888-8888-8888-888888888888', repeat('d', 64), '11111111-1111-1111-1111-111111111111', '2026-01-08T00:00:00Z', NULL, NULL, NULL, 1, '2026-01-01T00:00:00Z', '2026-01-01T00:00:00Z');"
run_alembic upgrade head
expect "0017_catalog_refresh_schedule" "SELECT version_num FROM alembic_version;"
expect "MEMBER" "SELECT target_role FROM account_invitations WHERE id = '88888888-8888-8888-8888-888888888888';"
query "DELETE FROM account_invitations WHERE id = '88888888-8888-8888-8888-888888888888';"
...
expect "NO" "SELECT is_nullable FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'account_invitations' AND column_name = 'target_role';"
