...
suffix="${$}-${RANDOM}"
network_name="wynterlabs-collection-value-history-${suffix}"
database_container="wynterlabs-collection-value-history-pg-${suffix}"
database_user="collection_value_history"
database_name="collection_value_history"
database_password="test-only-credential-2e5e3c1871f1"
...
docker run --rm --network "${network_name}" --volume "${api_root}:/app" --workdir /app \
  --env "CARDS_DATABASE_URL=${database_url}" --env "POSTGRES_TEST_DATABASE_URL=${database_url}" \
  ...
  "dd if=/dev/zero of=/tmp/mfa-test-key bs=32 count=1 status=none && pip install -q -e '.[dev]' && alembic upgrade head && python tests/postgres_collection_value_history_smoke.py"

echo "ephemeral-postgres-collection-value-history-smoke-ok"
